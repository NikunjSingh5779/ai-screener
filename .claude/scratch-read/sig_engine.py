...
class TradingSignal(BaseModel):
    ...
    @field_validator("timeframe", "symbol", "reasoning", "market", mode="before")
    @classmethod
    def _coerce_none_to_str(cls, v: object) -> str:
        """Free vision models often return null for fields they can't infer."""
        return v if isinstance(v, str) else ""

    def validate_action(self) -> "TradingSignal":
        """Reject unknown actions early (before it can reach the UI/logger)."""
        if self.action.lower() not in VALID_ACTIONS:
            raise ValidationError.from_exception_data(
                title="TradingSignal.action",
                line_errors=[
                    {
                        "type": "value_error",
                        "loc": ("action",),
                        "msg": f"action must be one of {sorted(VALID_ACTIONS)}",
                        "input": self.action,
                        "ctx": {"error": None},
                    }
                ],
